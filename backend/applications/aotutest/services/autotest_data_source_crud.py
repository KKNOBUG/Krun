...
@DateTime: 2026/3/6
参数化驱动：数据源（xlsx 解析结果）CRUD。存储颗粒度：一个文件对应多条记录，唯一 (case_id, step_no, step_code)。
"""
import traceback
...
def make_cache_key(case_id: int, step_no: int, step_code: str) -> str:
    """生成 Redis 缓存键：dataset_{case_id}_{step_no}_{step_code}。"""
    return f"dataset_{case_id}_{step_no}_{step_code}"


class AutoTestDataSourceCrud(ScaffoldCrud[AutoTestApiDataSourceInfo, AutoTestDataSourceCreate, AutoTestDataSourceUpdate]):
    """数据源 CRUD：唯一 (case_id, step_no, step_code)；同一文件多条记录共享 file_code。"""
    ...
    async def get_list_by_file_code(
        ...
        """按文件标识代码查询该文件下所有步骤记录，按 step_no 排序。"""
    ...
    async def get_by_case_step(
        ...
        """按 case_id + step_no + step_code 查询单条（用于按步骤取数）。"""
        if not case_id or step_no is None or not (step_code or "").strip():
            raise ParameterException(message="参数 case_id、step_no、step_code 不能为空")
        ...
        if not instance and on_error:
            raise NotFoundException(message=f"数据源(case_id={case_id}, step_no={step_no}, step_code={step_code})不存在")
        return instance
    ...
    async def create_data_sources_from_parsed(
        ...
        """上传时按 (case_id, step_no, step_code) 不存在则创建，存在则覆盖更新。"""
        if not case_id:
            raise ParameterException(message="参数 case_id 不能为空")
        if parsed_data is None:
            raise ParameterException(message="参数 parsed_data 不能为空")
        cache_key = make_cache_key(case_id, step_no, step_code)
        step_code = (step_code or "").strip()
        existing = await self.get_by_case_step(case_id=case_id, step_no=step_no, step_code=step_code, on_error=False)
        if existing:
            update_kw = {
                "file_name": file_name,
                "file_path": file_path,
        ...
        try:
            ...
            LOGGER.error(f"数据源创建失败 step_no={step_no} step_code={step_code}: {e}\n{traceback.format_exc()}")
    ...
    async def list_by_case(
        ...
        """按用例查询该用例下所有数据源记录（可能多文件、每文件多步骤），按 file_code、step_no 排序。"""
        return await self.model.filter(case_id=case_id, state=state).order_by("-updated_time", "step_no").all()

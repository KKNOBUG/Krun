...
class AutoTestApiEnvCreate(BaseModel):
    ...
    env_host: str = Field(..., max_length=255, description="环境主机(http|https://127.0.0.1:8000)")
...
class AutoTestApiEnvBase(BaseModel):
    ...
    env_host: Optional[str] = Field(None, max_length=255, description="环境主机(http|https://127.0.0.1:8000)")
...
class AutoTestApiEnvSelect(AutoTestApiEnvBase):
    ...
    created_user: Optional[UpperStr] = Field(None, max_length=16, description="创建人员")
    state: Optional[int] = Field(default=0, description="状态(-1:启用, 1:禁用)")

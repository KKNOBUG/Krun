...
class AutoTestApiConfigBase(BaseModel):
    ...
    config_params: Optional[Dict[str, Any]] = Field(None, max_length=16, description="数据库/服务器参数")
    config_kwargs: Optional[List[Dict[str, Any]]] = Field(None, max_length=64, description="通用环境变量配置")
    config_header: Optional[List[Dict[str, Any]]] = Field(None, max_length=64, description="通用请求头配置")
    ...
    database_type: Optional[AutoTestDataBaseType] = Field(None, max_length=16, description="数据库类型")
...
class AutoTestApiConfigUpdate(AutoTestApiConfigBase):
    config_id: Optional[int] = Field(None, ge=1, description="环境ID")
